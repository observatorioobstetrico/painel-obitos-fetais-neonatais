...
dados_obitos_fetais <- read.csv(gzfile("dados_oobr_obitos_fetais_2012_2023.csv.gz")) 

dados_obitos_neonatais <- read.csv(gzfile("dados_oobr_obitos_neonatais_2012_2023.csv.gz")) 
...
É um painel de visualização dinâmico com análises acerca da mortalidade de fetos e neonatos no Brasil, sob a perspectiva de algumas variáveis da base de dados do Sistema de Informações sobre Mortalidade (SIM). As informações consideradas são referentes aos registros de óbitos de fetos e neonatos dos anos de 2012 a 2023 **(sendo os dados de 2023 preliminares)**.
...
São providos do DATASUS do Ministério da Saúde e formam um grande banco anual composto por todos os registros das declarações de óbitos (o SIM) que está disponível a partir do ano de 1996. Atualmente, os dados do SIM estão disponíveis até 2023 **(sendo estes preliminares)**. Os dados deste painel são atualizados na última sexta feira de cada mês, com a última atualização tendo ocorrido em 31 de março de 2024.
...
dados_principais_filtrados <- reactive({
  ...
  if (input$selectNivelP == "Nacional") {
    dados_obitos_fetais_neonatais |>
      ...
      group_by(regiao, ano, capitulo_cid10, grupo_principais, causabas_categoria, tipo_do_obito, faixa_de_peso) |>
      summarise(obitos = sum(as.numeric(obitos))) |>
      ungroup()
    ...
    dados_obitos_fetais_neonatais |>
      ...
      group_by(uf, regiao, ano, capitulo_cid10, grupo_principais, causabas_categoria, tipo_do_obito, faixa_de_peso) |>
      summarise(obitos = sum(as.numeric(obitos))) |>
      ungroup() 
    ...
    dados_obitos_fetais_neonatais |>
      ...
      group_by(municipio, uf, regiao, ano, capitulo_cid10, grupo_principais, causabas_categoria, tipo_do_obito, faixa_de_peso) |>
      summarise(obitos = sum(as.numeric(obitos))) |>
      ungroup()
  }
})
...
observe({
  req(input$selectEstadoMuniP)
  print(data_plot_principais() |> filter(ano > 2017))
...
renderReactable({
  ...
  if (nrow(dados_principais_tabela()) > 0) {
    dados_principais_tabela() |>
      ...
      reactable(
        groupBy = c("capitulo_cid10", "grupo_principais", "causabas_categoria"),
        columns = list(
          capitulo_cid10 = colDef(
            name = "Capítulo CID-10",
            footer = "Total"
